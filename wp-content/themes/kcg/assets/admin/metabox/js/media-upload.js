; (function ($, window, document, undefined) {
    ...
    $.kcgMediaInsert = function (el, options) {
        this.el = el;
        this.$el = $(el);
        this.selectedItems = '';
        this.selectedUrl = [];
    ...
    $.kcgMediaInsert.prototype = {
        ...
        insertSavedItems: function () {
            var self = this;
            var con = self.$el.parent('.kcg-container');
            var selectedImage = con.find('.kcg-saved-val').val();
            if (selectedImage != '') {

                var savedItems = $.parseJSON(con.find('.kcg-saved-val').val())
            }
            savedItems = (savedItems) ? savedItems : [];
            var itemsCon = self.$el.prev('.selected-con');
            ...
            if (savedItems.length > 0) {
                if (self.options.fileType == "image") {
                    ...
                    for (var i in savedItems) {
                        ...
                        //self.selectedUrl
                        itemsCon.append('<div class="selected-image" id="img-' + i + '"><img src="' + savedItems[i]['thumb'] + '" class="kcg_images" data-full="' + savedItems[i]['full'] + '" data-itemid="' + savedItems[i]['itemId'] + '" data-id="' + [i] + '" alt=""><a href="#" class="remove">Remove</a></div>');
                    }
        ...
        openMedia: function () {
            var kcg_file_frame, attachment, self = this, container = this.$el.prev('.selected-con');
            ...
            this.$el.on('click', function (e) {
                ...
                kcg_file_frame = wp.media.frames.kcg_file_frame = wp.media({
                    title: uploaderTitle,
                    button: {
                ...
                kcg_file_frame.on('open', function () {
                    var selection = kcg_file_frame.state().get('selection');
                ...
                kcg_file_frame.on('select', function () {
                    ...
                    if (!self.options.multiSelect) {
                        attachment = kcg_file_frame.state().get('selection').first().toJSON();
                        self.selectedUrl = [];
        ...
        buildImageFrag: function (attachment, btn) {
            ...
            img = $('<img>', { src: imgUrl, class: 'kcg_images', 'data-full': fullSize, 'data-id': this.i, 'data-itemid': attachment.id });
        ...
        bindEvents: function () {
            ...
            $('.remove').on('click', function (e) {
                ...
                if (self.options.fileType == "image") {
                    ...
                    id = selImage.find('.kcg_images').data(id);
            ...
            this.$el.prev('.selected-con').sortable({
                ...
                update: function () {
                    ...
                    $.each(newOrder, function (i, item) {
                        var id = $('#' + item).find('.kcg_images').data('id');
                        if (oldOrder[id]) {
    ...
    $.fn.kcgMediaInsert = function (options) {

        return this.each(function () {
            // prevent multiple instantiation
            if (!$(this).data('kcgmediainsert'))
                $(this).data('kcgmediainsert', new $.kcgMediaInsert(this, options));
        });
    };
